...
Before **creating or substantially editing** agent-facing files — `.cursor/rules/*.mdc`, skills, `AGENTS.md`, `AI_INSTRUCTIONS.md`, or similar tool entry files:
...
- **Extend** an existing rule or `AGENTS.md` section when the concern fits
- **New file** only when the concern is distinct and would bloat an existing rule
