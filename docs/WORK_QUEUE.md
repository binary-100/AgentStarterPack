...
|----|------|--------|-------|
| WQ-426 | **Land the machine-local fix on the primary system and publish it** | **Next** | Instructions: the consolidated transfer document at the **transfer drive root** (2026-09-01) — untracked and unpathed here on purpose, since a tracked file must not record a drive mount. The code travels; the *other* repo's git index does not fix itself - three machine-local files are still tracked there and return on the next checkout unless untracked and staged. Step 50's index arm fails the suite if that is missed |
| WQ-415 | **Product-truth verify — limitations/capability prose vs code** | Queued | After WQ-426 - the transfer has to land before new engine work stacks on top of it. Promoted from Engineering backlog 2026-09-01; see `HANDOFF_WQ414` § Open engineering |
